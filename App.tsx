...
// Component to handle Aparat Embeds
const AparatEmbed = ({ html }: { html: string }) => {
  ...
  return <div ref={containerRef} className="w-full rounded-xl overflow-hidden my-4" />;
};
      ...
      <div className={`min-h-screen bg-white text-black p-8 ${fontFamily} ${dir === 'rtl' ? 'text-right' : 'text-left'}`}>
        <div className="max-w-[21cm] mx-auto space-y-12">
          ...
          <section className="print-break-inside-avoid">
            ...
            <p className="text-justify leading-relaxed">{content.about.text}</p>
          </section>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <Link to="/" className="flex-shrink-0 flex items-center gap-2 cursor-pointer group">
              <div className="w-8 h-8 bg-accent rounded-lg flex items-center justify-center text-white font-bold text-xl group-hover:bg-blue-700 transition-colors">
                M
              </div>
              <span className="font-bold text-xl tracking-tight text-slate-800 dark:text-white hidden sm:block">Madarshahi</span>
            </Link>
                      ...
                      <div className="flex flex-col md:flex-row gap-8 items-center">
                         <div className="w-full md:w-1/2">
                            <div className="relative rounded-2xl overflow-hidden shadow-lg aspect-video">
                                <img src={project.image} alt={project.title} className="w-full h-full object-cover" onError={(e) => e.currentTarget.src = "https://picsum.photos/600/400"} />
                            </div>
                         </div>
                         <div className="w-full md:w-1/2 space-y-4">
                             ...
                             {project.link && (
                               <a href={project.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center px-6 py-3 bg-accent hover:bg-blue-700 text-white rounded-lg font-bold transition-colors shadow-md mt-4">
                ...
                {content.inventions.items.map((item, idx) => (
                  <article key={idx} className="flex flex-col bg-slate-800 dark:bg-slate-900 rounded-2xl overflow-hidden shadow-xl">
                     ...
                     <div className="p-6 flex flex-col justify-center text-white">
                        ...
                        {item.link && (
                          <a href={item.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center text-sm font-medium text-white hover:text-blue-400 transition-colors">
                ...
                {content.publications.items.map((pub, idx) => (
                  ...
                  >
                    <div className="h-48 w-full bg-slate-100 dark:bg-slate-800 relative">
                      {pub.image ? (
                        <img src={pub.image} alt={pub.title} className="w-full h-full object-cover" onError={(e) => e.currentTarget.src = "https://picsum.photos/400/200"} />
                      ) : (
                    ...
                    <div className="p-6 flex-1 flex flex-col">
                       <h3 className="font-bold text-lg text-slate-800 dark:text-slate-100 mb-2 line-clamp-2">{pub.title}</h3>
                       {pub.authors && <p className="text-sm text-slate-500 dark:text-slate-400 mb-2">{pub.authors}</p>}
                       <p className="text-sm text-slate-600 dark:text-slate-300 line-clamp-3 mb-4 flex-1">{pub.description}</p>
...
const ProjectCard: React.FC<{ item: ProjectItem }> = ({ item }) => (
  <article className="group bg-white dark:bg-darkcard rounded-2xl overflow-hidden border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 hover:scale-[1.02] flex flex-col h-full">
    ...
    <div className="p-6 flex-1 flex flex-col">
      <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2 group-hover:text-accent dark:group-hover:text-blue-400 transition-colors">{item.title}</h3>
      <p className="text-slate-600 dark:text-slate-300 text-sm line-clamp-3 mb-4 flex-1">{item.description}</p>
      {item.link && (
        <a 
          href={item.link} 
          target="_blank" 
  ...
  <article className="bg-white dark:bg-darkcard p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm hover:border-accent/50 dark:hover:border-blue-500/50 transition-colors flex flex-col h-full">
    {item.videoEmbed ? (
       ...
       <div className="-mx-6 -mt-6 mb-4 h-48 overflow-hidden rounded-t-2xl relative group">
          <img src={item.image} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" onError={(e) => e.currentTarget.src="https://picsum.photos/400/300"} alt={item.title} />
       </div>
    )}
    ...
    <div className="flex flex-wrap gap-2 mb-4 text-xs">
       ...
       {item.location && <span className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 px-2 py-1 rounded">{item.location}</span>}
    ...
    {item.link && (
      <a href={item.link} target="_blank" rel="noreferrer" className="text-accent dark:text-blue-400 font-bold text-sm hover:underline mt-auto flex items-center gap-1">
